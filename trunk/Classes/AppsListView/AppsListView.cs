...
namespace Cameyo.OpenSrc.Client
{
    public partial class AppsListView : UserControl
    {
        ...
        private void appContextMenuLaunch_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in lvApps.SelectedItems)
            {
                AppItem appItem = (AppItem)item.Tag;
                if (appItem is DeployedAppItem)
                {
                    int exitCode = 0;
                    Utils.ExecProg(appItem.deployedApp.CarrierExeName, "", false, ref exitCode);
                }
            }
        }

        private void appContextMenuRemove_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("This will terminate and remove this app from your machine.\n" +
                "Any settings or data created in that app's sandbox will be removed.\n" +
                "Do you want to proceed?", "Confirmation", MessageBoxButtons.YesNo) != System.Windows.Forms.DialogResult.Yes)
                return;
            foreach (ListViewItem item in lvApps.SelectedItems)
            {
                AppItem appItem = (AppItem)item.Tag;
                int exitCode = 0;
                if (!Utils.ExecProg(appItem.deployedApp.CarrierExeName, "-quiet -remove", true, ref exitCode) || exitCode != 0)
                    MessageBox.Show("Error removing '" + appItem.deployedApp.AppID + "'");
            }
            RefreshApps();
        }

        private void appContextMenuEdit_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in lvApps.SelectedItems)
            {
                string exeName = Path.GetDirectoryName(System.Diagnostics.Process.GetCurrentProcess().MainModule.FileName);
                exeName = Path.Combine(exeName, "PackageEditor.exe");
                AppItem appItem = (AppItem)item.Tag;
                int exitCode = 0;
                if (!Utils.ExecProg(exeName, "\"" + appItem.deployedApp.CarrierExeName + "\"", false, ref exitCode) || exitCode != 0)
                    MessageBox.Show("Error editing '" + appItem.deployedApp.AppID + "'");
            }
        }
        ...
        private void appContextMenuProperties_Click(object sender, EventArgs e)
        {
            if (lvApps.SelectedItems.Count != 1)
                return;
            AppItem appItem = (AppItem)(lvApps.SelectedItems[0].Tag);
            AppPropertiesForm form = new AppPropertiesForm(appItem.deployedApp, imgsAppsLarge.Images[lvApps.SelectedItems[0].ImageIndex]);
            //Icon ico = Win32Function.getIconFromFile(deployedApp.CarrierExeName);
            //icon.Image = ico.ToBitmap();
            form.ShowDialog();
        }
